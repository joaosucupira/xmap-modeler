...
```
┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐
│   Frontend UI   │  │   Interface     │  │     Canvas      │
│   (Port 4500)   │  │   (Port 3000)   │  │   (Port 8080)   │
│     React       │  │     React       │  │     React       │
└─────────┬───────┘  └─────────┬───────┘  └─────────┬───────┘
          │                    │                    │
          └────────────────────┼────────────────────┘
...
| Frontend (3x React) | 512MB | 1GB | 2GB |
| PgAdmin | 128MB | 256MB | 500MB |
| **TOTAL** | **1.4GB** | **2.8GB** | **5.5GB** |

### Portas Necessárias

Certifique-se de que estas portas estão disponíveis:
- `3000` - Interface Principal
- `4500` - Frontend UI
- `5432` - PostgreSQL
- `5050` - PgAdmin
- `8000` - API Backend
...
DEBUG=true
CORS_ORIGINS=["http://localhost:3000","http://localhost:4500","http://localhost:8080"]
```
...
|---------|-----|-----------|
| **Interface Principal** | http://localhost:3000 | Interface principal do sistema |
| **Frontend UI** | http://localhost:4500 | Interface alternativa/moderna |
